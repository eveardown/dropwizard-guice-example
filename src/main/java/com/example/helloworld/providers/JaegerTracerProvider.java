...
public class JaegerTracerProvider implements TracerProvider {
	...
	private void initTracer() {
		...
		Configuration config = new Configuration("jaeger-tracer-service")
				.withSampler(samplerConfig).withReporter(reporterConfig);
		this.jtracer = config.getTracer();
		logger.info("________________________Tracer is inirialized from the Provicer ________________________");
	}
